...
# Get total variance explained VALUES by all factors for each view
...
### CREATE TABLE
### And cut it to 10F
# First create mofa_results_simple from final_table
mofa_results_simple <- data.frame(
  Factor = final_table$Factor,
  F_stat = final_table$F.statistic,
  P_value = final_table$P.value,
  Significance = final_table$Significance
)
...
kable(mofa_r_squared_simple, 
      ...
      row.names = FALSE,
      col.names = c("Factor", "View", "Feature", "R2", "P-value", "Significance")) %>%
...
write.csv(mofa_r_squared_simple, 
          "C:/Users/sandr/OneDrive - University of Glasgow/PROJECT/DATA/INTEGRATION/mofa_r_squared_table.csv", 
          row.names = FALSE)


########################################################################
#UPDATED POST UMITOOLS RSULTS SUGGEST F2 F4 to be of interest
#########################################################################

######################################################
## SUMMARY FOR F2 ABOVE  ( NEW MOST SIGNIFICANT ) ##
#####################################################

#############################
## SUMMARY FOR FACTOR 4 (SECOND MOST SIG) ##
#############################
# F4 Second strongest genotype effect (p = 0.005)

# Get top features for Factor 4
top_micro_f4 <- get_top_features(model, 4, "micro", 10)
top_fly_f4 <- get_top_features(model, 4, "fly", 10)

# Panel A: Factor activity by genotype
panel_a_f4 <- plot_factor(model, 
                          factors = 4,
                          color_by = "mitonuclear",
                          add_violin = TRUE,
                          dodge = TRUE) + 
  scale_fill_manual(values = geno_colors) +
  scale_color_manual(values = geno_colors) +
  ggtitle("Factor 4 Activity by Genotype") +
  theme_minimal() +
  theme(legend.position = "bottom",
        plot.title = element_text(hjust = 0.5))

# Panel B: Microbiome weights
panel_b_f4 <- ggplot(top_micro_f4, aes(x = reorder(feature, abs_value), y = value)) +
  geom_col(aes(fill = value > 0), width = 0.7) +
  scale_fill_manual(values = c("TRUE" = "darkred", "FALSE" = "blue")) +
  coord_flip() +
  labs(title = "Top Microbiome Features",
...
# Panel C: Transcriptome weights
panel_c_f4 <- ggplot(top_fly_f4, aes(x = reorder(feature, abs_value), y = value)) +
  geom_col(aes(fill = value > 0), width = 0.7) +
  scale_fill_manual(values = c("TRUE" = "darkred", "FALSE" = "blue")) +
  coord_flip() +
  labs(title = "Top Transcriptome Features",
...
# Panel D: Correlations
cor_matrix_f4 <- create_cross_correlations(model, 4, 5)
cor_df_f4 <- expand.grid(Microbiome = rownames(cor_matrix_f4),
                         Transcriptome = colnames(cor_matrix_f4))
cor_df_f4$Correlation <- as.vector(cor_matrix_f4)

panel_d_f4 <- ggplot(cor_df_f4, aes(x = Microbiome, y = Transcriptome, fill = Correlation)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", 
                       midpoint = 0, limits = c(-1, 1)) +
  geom_text(aes(label = round(Correlation, 2)), size = 3) +
...
# Panel E: Microbiome scatter plots
panel_e_f4 <- plot_data_scatter(model,
                                view = "micro",         
                                factor = 4,             
                                features = 5,
                                add_lm = TRUE,
  ...
  ggtitle("Microbiome Features vs Factor 4") +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12),
    strip.text = element_text(size = 5.6), 
    axis.text = element_text(size = 8)
...
# Panel F: Transcriptome scatter plots
panel_f_f4 <- plot_data_scatter(model,
                                view = "fly",         
                                factor = 4,             
                                features = 5,
                                add_lm = TRUE,
  ...
  ggtitle("Transcriptome Features vs Factor 4") +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12),
    strip.text = element_text(size = 8),  
    axis.text = element_text(size = 8)
...
# Combine all panels for Factor 4
combined_plot_f4 <- grid.arrange(
  panel_a_f4, panel_b_f4,
  panel_c_f4, panel_d_f4,
  panel_e_f4, panel_f_f4,
  ncol = 2, nrow = 3,
  heights = c(1, 1, 1.2),
  top = textGrob("Factor 4 Analysis - Second Genotype Association", 
                 gp = gpar(fontsize = 16, fontface = "bold"))
)

#################################################################
# Updated R-squared analysis for Factors 2 and 4
#################################################################
# Extract R squared values for the genotype-associated factors
r_squared_table_sig <- data.frame()

# For Factors 2 and 4
for(factor_num in c(2, 4)) {
  # Microbiome
  micro_r2 <- extract_r_squared(model, factor_num, "micro", 5)
  r_squared_table_sig <- rbind(r_squared_table_sig, micro_r2)
  
  # Transcriptome
  fly_r2 <- extract_r_squared(model, factor_num, "fly", 5)
  r_squared_table_sig <- rbind(r_squared_table_sig, fly_r2)
}

# Clean up the table
r_squared_table_sig$feature <- gsub("_micro|_fly", "", r_squared_table_sig$feature)
r_squared_table_sig$r_squared <- round(r_squared_table_sig$r_squared, 3)
r_squared_table_sig$p_value <- round(r_squared_table_sig$p_value, 4)

# Add significance codes
r_squared_table_sig$significance <- ifelse(r_squared_table_sig$p_value < 0.001, "***",
                                           ifelse(r_squared_table_sig$p_value < 0.01, "**",
                                                  ifelse(r_squared_table_sig$p_value < 0.05, "*", "ns")))

# Sort by factor and R-squared
r_squared_table_sig <- r_squared_table_sig[order(r_squared_table_sig$factor, -r_squared_table_sig$r_squared), ]

# Rm NA values
r_squared_table_sig <- r_squared_table_sig[!is.na(r_squared_table_sig$r_squared), ]

print(r_squared_table_sig)

# Create formatted table
mofa_r_squared_sig <- data.frame(
  Factor = r_squared_table_sig$factor,
  View = r_squared_table_sig$view,
  Feature = r_squared_table_sig$feature,
  R_squared = r_squared_table_sig$r_squared,
  P_value = ifelse(r_squared_table_sig$p_value < 0.001, 
                   "< 0.001",
                   r_squared_table_sig$p_value),
  Significance = r_squared_table_sig$significance
)

kable(mofa_r_squared_sig, 
