...
samp$mitonuclear <- with(samp, factor(paste(mito, nuclear, sep = "")))
...
### OUTPUT DIR
OUT_DIR <- "C:/Users/sandr/OneDrive - University of Glasgow/PROJECT/DATA/TRANSCRIPTOME"
...
### PCA not used but retained as exploratory
...
pheatmap(top_de_matrix_log,
         ...
         clustering_distance_cols = "correlation",
         border_color = NA,
         main = "A  Top 100 Differentially Expressed Genes",
         fontsize = 8,
         fontsize_row = 4,
         height = 12 / 2.54, width = 10 / 2.54,   # ~10x12 cm in in
         filename = file.path(OUT_DIR, "Fig2A_top100_DE_heatmap.pdf"))
...
u4 <- ggplot(umap_df, aes(UMAP1, UMAP2, color = mitonuclear)) +
  geom_point(size = 3, alpha = 0.7) +
  scale_color_manual(values = geno_colors_msb, name = "Genotype") +
  theme_msb() +
  ggtitle("B  UMAP by Mitonuclear Genotype") +
  xlab("UMAP1") +
  ylab("UMAP2")
...
# perm addative
perm_additive <- adonis2(dist_matrix ~ nuclear + mito, data = samp, permutations = 999, by = "terms")
print(perm_additive)

# test for interaction effects
perm_mitonuclear <- adonis2(dist_matrix ~ nuclear * mito, data = samp, permutations = 999, by = "terms")
...
#mitonuclear_results$Factor <- gsub("\u00d7", "x", mitonuclear_results$Factor)
mitonuclear_results$Factor <- gsub("Nuclear (.) Mitochondrial", "Nuclear x Mitochondrial", mitonuclear_results$Factor)
...
###############################################
## PAPER FIGURES - FIGURE 2
###############################################

# theme_msb(), geno_colors_msb, OUT_DIR, and the Fig2A heatmap save are all
# defined/handled near the top of this script
...
u2_msb <- u2 + theme_msb() + labs(title = "D  UMAP by nDNA")
u3_msb <- u3 + theme_msb() + labs(title = "C  UMAP by mtDNA")

fig2b <- u4 | u3_msb | u2_msb

save_plot(fig2b, "Fig2B_umap_panels.pdf", width = 30, height = 10)


## Fig2C: PERMANOVA - population 
...
pop_titled <- arrangeGrob(
  textGrob("C  PERMANOVA: Gene expression ~ population (Bray-Curtis)",
           x = 0, hjust = 0,
...
ggsave(file.path(OUT_DIR, "Fig2C_permanova_population.pdf"),
       plot = pop_titled, width = 16, height = 5,
       units = "cm", device = "pdf", useDingbats = FALSE)


## Fig2D: PERMANOVA - mitonuclear (additive + interaction)
...
mito_titled <- arrangeGrob(
  textGrob("D  PERMANOVA: Gene expression ~ mitonuclear genotype (Bray-Curtis)",
           x = 0, hjust = 0,
           gp = gpar(fontsize = 9, fontface = "bold", fontfamily = "sans")),
  mito_grob,
  nrow = 2, heights = c(0.15, 1)
...
ggsave(file.path(OUT_DIR, "Fig2D_permanova_mitonuclear.pdf"),
       plot = mito_titled, width = 16, height = 6,
       units = "cm", device = "pdf", useDingbats = FALSE)

message("All Figure 2 outputs saved to: ", OUT_DIR)
